...
player_rect = player.get_rect(center=(100, HEIGTH // 2))
...
def create_enemy(): 
    ...
    enemy_rect = pygame.Rect(WIDTH -100, random.randint(100, HEIGTH - 100), *enemy_size)
...
def create_bonus():
    ...
    bonus_rect = pygame.Rect(random.randint(100, WIDTH - 200), 100 , *bonus_size)
...
while playing: 
    ...
    for e in pygame.event.get():
        ...
        if e.type == CREATE_ENEMY:
            enemies.append(create_enemy())

        if e.type == CREATE_BONUS:
            bonuses.append(create_bonus())

        if e.type == CHANGE_IMAGE:
